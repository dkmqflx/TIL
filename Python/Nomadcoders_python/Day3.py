...
def add_to_dict(*args):
    if type(args[0]) != dict:
        print(f"You need to send a dictionary. You sent: {str(type(args[0]))}")
        return
    if len(args) != 3:
        print("You need to send a word and a definition")
        return
    
    if not args[0].get(args[1]):
...
def get_from_dict(*args):
    if type(args[0]) != dict:
        print(f"You need to send a dictionary. You sent: {str(type(args[0]))}")
        return
    if not str in [type(x) for x in args]:
        print("You need to send a word to search for")
        return
    
    if args[1] in args[0]:
        print(f"{args[1]}: {args[0].get(args[1])}")
    else:
...
def update_word(*args):
    ...
    if not args[1] in args[0]:
        print(f"{args[1]}is not on the dict. Can't update non-existing word")
    else:
        args[0][args[1]] = args[2]
        print(f"{args[1]} has been updaed to: {args[0].get(args[1])}")
...
def delete_from_dict(*args):
    ...
    if not args[1] in args[0]:
        print(f"{args[1]}is not on the dict. Won't delete")
    else:
        del(args[0][args[1]])
