...
- **각도**: DBA/엔진 내부가 아니라 **앱 개발자 실전**. **2026 기준(PostgreSQL 18, 2025-09 릴리스 — 현재 최신 안정 버전)** 실무 트렌드 반영: 네이티브 `uuidv7()`, virtual generated columns, `RETURNING`의 `OLD`/`NEW`, 비동기 I/O, keyset pagination, pgvector 등.
...
| PostgreSQL 버전 | **PostgreSQL 18** (`postgres:18` 이미지) — 2026 현재 최신 안정 버전 |
| 실습 환경 | **Docker + psql 순수 SQL 중심** — `docker compose`로 Postgres 18 띄우고 psql로 직접 SQL 실행 |
...
- `03` ⭐ **연결 풀링(PgBouncer · 풀 사이징 · 서버리스 connection storm)** · 확장(pgcrypto · **pgvector**) · 백업(`pg_dump` · **증분 백업 `pg_basebackup --incremental`**) · 회고 & 다음 로드맵(파티셔닝/논리 복제 등)

## 7. 스코프 결정 (포함/제외)

- **포함**: 위 18편. 2026 실무 디테일(PG18 네이티브 `uuidv7()`, virtual generated columns, `RETURNING OLD/NEW`, 비동기 I/O, temporal 제약, timestamptz 타임존, keyset pagination, pgvector) 주입.
